...
export class View {
    ...
    public buildHomePage() {
        ...
        container.html(
            '<div class="scms-landing-image" style="height:600px;background:url(vicri.jpg) no-repeat;background-position:center;background-size:cover;position:relative;"></div>'
        );
        ...
        photos_block
            ...
            .addClass('scms-content-block-paragraph')
            .html(
                "Vous retrouverez, ici, organisées par dossier triés par date, toutes les photos réalisées par le groupe vicri de l'INDSé 2e&3e degrés."
            );
        ...
        videos_block
            ...
            .addClass('scms-content-block-paragraph')
            .html(
                "Vous retrouverez, ici, toutes les vidéos réalisées par le groupe vicri de l'INDSé 2e&3e degrés qui ont été uploadées sur YouTube"
            );
    ...
    public buildProjectsPage(
        getProjects: (callback: (projects?: Array<Project>) => void) => void
    ) {
        let container: ExtJsObject = this._c;

        container.html(
            '<div class="scms-landing-image" style="height:600px;max-height:calc(100vh - 60px);background:url(./res/projects.jpg) no-repeat;background-position:center;background-size:cover;position:relative;"></div>'
        );
        ...
        let wrapper = project_block.child('div').addClass('wrapper');
        ...
        getProjects(projects => {
            projects.forEach(project => {
                ...
                let pb = wrapper.child('div').addClass('project');

                let pbc: ExtJsObject = pb.child('div').addClass('img');

                pbc
                    .child('div')
                    .css(
                        'background',
                        'url(https://picsum.photos/450/300?random#' +
                            Math.random() +
                            ') no-repeat'
                    )
                    .css('background-position', 'center')
                ...
                let cf = pb.child('div').addClass('clearfix');
    ...
    public buildVideosPage(
        getProjects: (callback: (projects?: Array<any>) => void) => void
    ) {
        let container: ExtJsObject = this._c;

        container.html(
            '<div class="scms-landing-image" style="height:600px;max-height:calc(100vh - 60px);background:url(./res/our_videos.jpg) no-repeat;background-position:center;background-size:cover;position:relative;"></div>'
        );
            ...
            projects.forEach(project => {
                ...
                pb
                    ...
                    .html(
                        `<b>${
                            project.title
                        }</b><br /><div style="height:5px;"></div>${nl2br(
                            project.description,
                            false
                        )}`
                    );
    ...
    public buildPhotosPage(
        getProjects: (callback: (projects?: Array<any>) => void) => void
    ) {
        let container: ExtJsObject = this._c;

        container.html(
            '<div class="scms-landing-image" style="height:600px;max-height:calc(100vh - 60px);background:url(./res/our_photos.jpg) no-repeat;background-position:center;background-size:cover;position:relative;"></div>'
        );
    ...
    public buildProjectPage(
        getProject: (
            id: string,
            callback: (data: Project) => void,
            error: (error: Project) => void
        ) => void,
        id: string
    ) {
        let container: ExtJsObject = this._c;
        ...
        getProject(
            ...
            error => {
                project_block.html(
                    `Une erreur est survenue : il se peut que le projet n'existe pas<br /> ${
                        error.message
                    }`
                );
            }
    ...
    public set container(c: ExtJsObject) {
        this._c = c;
    }
    public set page(p: Page) {
        this._page = p;
    }
}
